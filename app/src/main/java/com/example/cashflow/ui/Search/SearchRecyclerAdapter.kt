...
class SearchRecyclerAdapter(private val activity: Activity) : RecyclerView.Adapter<SearchRecyclerAdapter.ViewHolder>() {

    private var bills: List<Bill> = emptyList()
    private val user: User = User(FirebaseAuth.getInstance().currentUser!!.phoneNumber!!)

    init{
        update(RequestingStage())
    }
    fun update(stage: BillState) {
        user.getBillsAtStage(stage.id) { fetchedBills ->
